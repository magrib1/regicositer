...
class Pengabdian extends CI_Controller {
	...
	public function index(){
		$data = array(
			'page' => 'pengabdian_user',
			'link' => 'pengabdian',
			'script' => 'script/pengabdian_user',
			'list' => $this->Model->ambil_banyak_kondisi('tb_usulan_proposal', array('jenis' => 'pengabdian', 'ketua_peneliti' => $this->session->userdata('id'),'delete' => '0'))->result(),
		);
    ...
    public function auto_prodi(){
        ...
        $data = $this->M_simuk->like('tb_unit', 'nama_unit', $keyword)->result_array();   
        echo json_encode($data);
    }
	...
	public function usulan($id){
		...
		$this->load->view('template/wrapper',$data);
            ...
            echo'<script>window.location.href="'.base_url().'/user/pengabdian";</script>';
		}
	}
	...
	public function simpan_afiliasi_dosen(){
		...
		// $cek_anggota = $this->Model->list_join_where_banyak('tb_usulan_proposal','tb_afiliasi_dosen','tb_usulan_proposal.id_proposal=tb_afiliasi_dosen.id_proposal', array('delete' => '0', 'status' => 'diterima', "id_anggota" => $dosen[0] ) )->num_rows();

		// $cek_ketua = $this->Model->ambil_banyak_kondisi('tb_usulan_proposal', array('ketua_peneliti' => $dosen[0], 'delete' => '0', 'status' => 'diterima', 'didanai' => 'didanai') )->num_rows();
   ...
   //          if (!is_dir('./assets/file_upload/'.$path) ) {
	...
	public function cetak_pengesahan($id){
		...
		$data = array(
			'page' => 'cetak_pengesahan',
			'link' => 'pengabdian',
			'script' => 'script/pengabdian_user',
			'list' => $this->Model->ambil('id_proposal',$id,'tb_usulan_proposal')->row(),
		);
		$this->load->view('cetak_pengesahan', $data);
